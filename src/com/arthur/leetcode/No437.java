...
public class No437 {
    ...
    private int recursionPathSum(TreeNode root, HashMap<Integer, Integer> sumMap, int target, int currentSum) {
        ...
        currentSum += root.val;
        int res = 0;
        res += sumMap.getOrDefault(currentSum - target, 0);
        sumMap.put(currentSum, sumMap.getOrDefault(currentSum, 0) + 1);
        res += recursionPathSum(root.left, sumMap, target, currentSum);
        res += recursionPathSum(root.right, sumMap, target, currentSum);
